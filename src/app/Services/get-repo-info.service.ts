  ...
  searchRepo ( repo: string ) {
    ...
    promise.then( async ( data: any ) => {
      await data.items.map( ( obj: any ) => {
        ...
        this.results.push(new Repomodel( owner, repoUrl, description, size, language, license, allowForking, visibility, forks ))
      } )
